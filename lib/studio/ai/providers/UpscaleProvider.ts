import { IUpscaleProvider, AIProviderResponse } from "../AIProviderManager";
import { API_BASE } from "@/lib/api";

export class NextApiUpscaleProvider implements IUpscaleProvider {
  async upscale(image: File, scale?: number): Promise<AIProviderResponse<string>> {
    try {
      const formData = new FormData();
      formData.append("job_type", "upscale");
      formData.append("file", image);
      if (scale) formData.append("extra_arg", scale.toString());
      ...
      if (!res.ok) {
        ...
        throw new Error(`Falha ao iniciar upscale: ${errorText}`);
      }
      ...
      console.error("[NextApiUpscaleProvider]", error);
    ...
    while (retries < maxRetries) {
      ...
      try {
        ...
        if (data.status === 'completed' && data.result_url) {
          ...
          return { success: false, error: data.error_message || "Erro desconhecido no upscale." };
        }
      } catch (e) {
